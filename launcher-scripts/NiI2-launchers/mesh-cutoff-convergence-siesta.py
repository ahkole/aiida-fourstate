...
def launch_magnetic_exchange_calculation(neigh_idx = 1, ask_for_confirmation=True):
    ...
    site2, dist = find_neighbor(site1 = site1, neigh_idx=neigh_idx, filter_element="Ni", atoms=structure_supercell.get_ase())

    magnetization_per_site = [2. if structure_supercell.get_kind(kind_name).symbol == "Ni" else 0. for kind_name in structure_supercell.get_site_kindnames()]
    magnetization_magnitude = 2.0 # For the two sites to flip
    ...
    if ask_for_confirmation:
        print(f"Submitting J({neigh_idx}) for NiI2 to {code_label} on {num_machines} node(s) with {num_mpiprocs_per_machine=}.")
    ...
    node.label = f"NiI2 J({neigh_idx}) calculation"
