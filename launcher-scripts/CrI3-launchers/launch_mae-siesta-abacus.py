...
abacus_basis = orm.load_node(uuid='105d1c79-4596-4ada-89de-1d546ccc3117')
...
def launch_mae_calculation(ask_for_confirmation=True):
    ...
    magnetic_sites = [structure_unitcell.get_kind(kind_name).symbol == "Cr" for kind_name in structure_unitcell.get_site_kindnames()]
    magnetic_sites_idx = np.flatnonzero(magnetic_sites)
    magnetization_magnitude = 3.0 
    ...
    if ask_for_confirmation:
        print(f"Submitting MAE for CrI3 to {code_label} on {num_machines} node(s) with {num_mpiprocs_per_machine=}.")
