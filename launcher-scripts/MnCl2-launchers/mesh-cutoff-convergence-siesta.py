...
max_memory_kb = 377487360  # 2/8 of total memory of 192-core fat_genoa node
...
def launch_magnetic_exchange_calculation(neigh_idx = 1, ask_for_confirmation=True):
    ...
    ase_atoms = ase.io.read('mncl2_2x1_15A.cif')
    ...
    site2, dist = find_neighbor(site1 = site1, neigh_idx=neigh_idx, filter_element="Mn", atoms=structure_supercell.get_ase())

    magnetization_per_site = [5. if structure_supercell.get_kind(kind_name).symbol == "Mn" else 0. for kind_name in structure_supercell.get_site_kindnames()]
    magnetization_magnitude = 5.0 # For the two sites to flip
    ...
    optimization_inputs = {
        ...
        'engine_kwargs': orm.Dict(dict=dict(
            ...
            tol=etol,
            input_key="create.generator_inputs.custom_protocol.parameters:mesh-cutoff",
            result_key="evaluate.result",
            convergence_window=conv_window
            )),
        'evaluate_process': CreateEvaluateWorkChain,
        'evaluate': {
            'create_process': MagneticExchangeWorkChain,
            'create': inputs,
            'evaluate_process': compute_FM_AFM_diff,
            'evaluate': dict(num_effective_NN=num_effective_NN, num_magnetic_atoms=num_magnetic_atoms),
            'output_input_mapping': orm.Dict(dict={'energy_upup': 'energy_upup', 'energy_updown': 'energy_updown'}),
        },
    }

    if ask_for_confirmation:
        print(f"Submitting J({neigh_idx}) for MnCl2 to {code_label} on {num_machines} node(s) with {num_mpiprocs_per_machine=}.")
    ...
    node.label = f"MnCl2 J({neigh_idx}) calculation"
